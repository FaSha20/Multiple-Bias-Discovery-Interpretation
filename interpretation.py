...
class ComponentRetriever(object):
    ...
    def download_save_retrieval_result(self, retrieval_results, save_path):
      ...
      for img_id, result in enumerate(retrieval_results):
          try:
              res = requests.get(result['url'], stream=True, timeout=10)
          ...
          img_file = os.path.join(save_path, f'img_{img_id}.jpg')
          with open(img_file, 'wb') as file:
              for chunk in res.iter_content(1024):
                  file.write(chunk)

          # Save metadata immediately
          meta_file = os.path.join(save_path, 'meta', f'img_{img_id}.json')
          with open(meta_file, 'w') as file:
              json.dump(result, file)

          print(f"Saved {img_file}")

    def download_save_retrieval_result0(self, retrieval_results, save_path):
        os.makedirs(save_path, exist_ok=True)
        os.makedirs(f'{save_path}/meta', exist_ok=True)
        for img_id, result in enumerate(retrieval_results):
            try:
    ...
    def retrieval0(self, embedding, save_path):
    ...
    def retrieval(self, embedding, save_path, top_k=10):

      os.makedirs(save_path, exist_ok=True)

      embedding = F.normalize(embedding, dim=-1)
      embedding = embedding.detach().cpu().numpy().astype("float32").reshape(1, -1)
      ...
      for rank, idx in enumerate(indices[0]):
          ...
          dst_path = os.path.join(save_path, f"rank_{rank}.jpg")

          # copy image
          import shutil
          shutil.copy(src_path, dst_path)

      print(f"Saved top-{top_k} retrievals to {save_path}")
...
if __name__ == "__main__":
    ...
    # image_captioner.run()
    component_path = "save0/002_multi_bias_identification/result/component.csv"
    save_path = "save0/004_component_interpretion/clip_retrieval"

    retriever = ComponentRetriever(args, component_path, save_path)
    retriever.run(methods=["DIM"])   # run only DIM components
