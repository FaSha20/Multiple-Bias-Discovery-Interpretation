...
class Trainer(object):
    ...
    def _compute_supervision(self):
        ...
        for i in range(self.args.num_epochs):
            classifier.load_state_dict(torch.load(os.path.join(self.args.model_path[:-3], f'epoch{i}.pt'), map_location=self.device))
    ...
    def _compute_component(self):
        ...
        for idx_target, super_class in enumerate(self.super_class_names):
            ...
            if self.mode == "pls":
              ...
              pca_comps = self.pca_components(cur_feature_list, NUM_COMPONENTS)
              component_df = self.store_component(component_df, 'PCA', super_class, pca_comps)
    ...
    def run(self):
        os.makedirs(f"{self.save_path}/result", exist_ok=True)
    ...
    def pca_components(self, feature, num_component):
        # feature: (n_samples, d)
        pca = PCA(n_components=num_component, svd_solver="auto", random_state=0)
        pca.fit(feature)

        # components_: (num_component, d)
        comps = torch.from_numpy(pca.components_).to(self.device).float()
        return comps
