...
class DataLoaderCreator(object):
    ...
    def train_data_loader(self, dataset_name=None, batch_size=None, transform=None, shuffle=None, sampler=None):
        ...
        if dataset_name == 'mnist':
          ...
          trainset = ImbalancedSuperCIFAR100(split='intervention', root="data", transform=transform, download=True)
          train = DataLoader(trainset, batch_size=batch_size, shuffle=shuffle, num_workers=4, pin_memory=True)
        ...
        if self.train_dataset is None:
          ...
          # Create deterministic split ONCE
        ...
        # Replace dataset with train subset
        train = DataLoader(
            self.train_subset,
            batch_size=batch_size,
            shuffle=shuffle,
            num_workers=getattr(train, "num_workers", 0),
            pin_memory=getattr(train, "pin_memory", False),
        )
        return train
